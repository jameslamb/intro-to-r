...
NA` can actually hold valuable information
For example, imagine that you get a dump of data from Salesforce or some other CRM system with information like customer_name, date_of_first_contact, and date_of_second_contact.

<br>

Depending on how the system was set up, `date_of_second_contact1` may have dates only for customers who have been contacted at least twice, and be `NA` everywhere else.
This is valuable information!
If you want to build a model of 1-contact conversion, you could use the presence/absence of `NA` to help you identify the 1-contact customers that belong in your model.
...
It may be useful to try an [imputation strategy](https://www.analyticsvidhya.com/blog/2016/03/tutorial-powerful-packages-imputing-missing-values/) to replace `NA` values with reasonable alternatives.
...
You first create a simple plot, then "add on" a legend, more variables, other plot types, etc.
...
You can combine multiple plots in a grid layout.
See ["The Base Plotting System"](../code/programming-supplement.html#The_Base_Plotting_System) for an example.
...
It's common in data science workflows to have two mismatched tables of data from different sources and to want to combine them by matching on one or more keys.
...
But what if you want to merge rows instead of columns?
...
- You need to choose at least one data retrieval package, one statistics package, and one visualization package from [this list](../assignments/final_project_packages.md)
...
**Paths:** [Relative vs absolute](https://www.redhat.com/en/blog/linux-path-absolute-relative) | [listing files in a directory in R](https://stat.ethz.ch/R-manual/R-devel/library/base/html/list.files.html)
