...
```{r calculateDates, echo = FALSE}
fp_pres_dt <- as.Date("2026-08-08")

# full month name, day, 4-digit year
fmt <- "%B %d, %Y"

due_dates <- list(
    "survey"         = format(fp_pres_dt - 28L, fmt)
    , "quiz1"        = format(fp_pres_dt - 21L, fmt)
    , "quiz2"        = format(fp_pres_dt - 14L, fmt)
    , "assignment1"  = format(fp_pres_dt - 14L, fmt)
    , "assignment2"  = format(fp_pres_dt - 7L, fmt)
    , "fp_proposal"  = format(fp_pres_dt - 7L, fmt)
    , "fp_report"    = format(fp_pres_dt, fmt)
    , "fp_pres"      = format(fp_pres_dt, fmt)
    , "fp_script"    = format(fp_pres_dt, fmt)
    , "extra_credit" = format(fp_pres_dt + 3L, fmt)
)
```
...
**YOU WILL PRESENT YOUR FINAL PROJECT ON A VIDEO CALL SATURDAY, `r due_dates[["fp_pres"]]`, 9:00a-12:00P CT**.
...
6. **(due `r due_dates[["survey"]]`)** Take the `pre-course survey` on D2L
...
9. **(due `r due_dates[["quiz1"]]`)** Complete `Quiz 1`
...
12. **(due `r due_dates[["quiz2"]]`)** Complete `Quiz 2`
13. **(due `r due_dates[["assignment1"]]`)** Submit `Programming Assignment 1`
...
17. **(due `r due_dates[["assignment2"]]`)** Submit `Programming Assignment 2`
18. **(due `r due_dates[["fp_proposal"]]`)** Submit `Final Project Proposal`
...
21. **(due `r due_dates[["fp_script"]]`)** Submit `Final Project Script(s)`
22. **(due `r due_dates[["fp_report"]]`)** Submit `Final Project Written Report`
23. **(due `r due_dates[["fp_pres"]]`)** Attend `Final Project Presentation` (video call)
  * present your final project code to your classmates
  * provide peer review feedback to your classmates
24. **(due `r due_dates[["extra_credit"]]`)** *(optional)* complete `Extra Credit Assignment`
...
|Quiz              |Opens        |Due                                     |
|:----------------:|:-----------:|:--------------------------------------:|
|Pre-course survey | Immediately | 11:00p CT on `r due_dates[["survey"]]` |
|Quiz 1            | Immediately | 11:00p CT on `r due_dates[["quiz1"]]`  |
|Quiz 2            | Immediately | 11:00p CT on `r due_dates[["quiz2"]]`  |
...
|Quiz                      |Opens        |Due                                          |
|:------------------------:|:-----------:|:-------------------------------------------:|
|Programming Assignment 1  | Immediately | 11:00p CT on `r due_dates[["assignment1"]]` |
|Programming Assignment 2  | Immediately | 11:00p CT on `r due_dates[["assignment2"]]` |
...
|Assignment                 | Due                                        |
|:-------------------------:|:------------------------------------------:|
|Final Project proposal     |11:00p CT on `r due_dates[["fp_proposal"]]` |
|Final Project script(s)    |11:00p CT on `r due_dates[["fp_script"]]`   |
|Final Project report       |11:00p CT on `r due_dates[["fp_report"]]`   |
|Final Project presentation |live, 9:00a CT `r due_dates[["fp_pres"]]`   |
...
|Assignment              | Due                                         |
|:----------------------:|:-------------------------------------------:|
|Extra Credit Assignment |11:59p CT on `r due_dates[["extra_credit"]]` |

## D. Deadlines and Late Policy

Any work submitted after a deadline, without an approved extension, will receive a grade of 0.

The assignments are teaching tools and each is accompanied with a lecture video explaining the answers.
These videos are released promptly after assignments are due, so students have time to watch and learn from them.
...
For more details, see https://bulletin.marquette.edu/policies/grade-appeals/
...
If you use those technologies, I encourage you to only use them for limited debugging help, similar to the way you'd use a search engine or conversation with a trusted colleague.
...
See https://www.marquette.edu/central/registrar/2026-summer-academic-calendar.php for dates like the deadlines for dropping or withdrawing from the course.
